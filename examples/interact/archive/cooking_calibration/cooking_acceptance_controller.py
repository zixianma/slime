...
def stages(checkpoint):
    return [
        ...
        ('02-train', ['bash','examples/interact/archive/cooking_calibration/run_cooking_calibration.sh']+EXTRA_ARGS, 5700, {}),
        ('03-restore', ['bash','examples/interact/archive/cooking_calibration/run_cooking_calibration.sh']+EXTRA_ARGS+
         ['--load',checkpoint,'--use-checkpoint-opt-param-scheduler'], 600, {'COOKING_RESTORE_ONLY':'1'}),
        ('04-verify', ['python','examples/interact/archive/cooking_calibration/cooking_calibration_verify.py'], 120, {}),
    ]
