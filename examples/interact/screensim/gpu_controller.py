...
code = stage("preflight", [sys.executable, "examples/interact/screensim/screensim_preflight.py"], 900)
if code:
    sys.exit(code)
if os.environ.get("INTERACT_SMALL_INTIME_TEST") == "1":
    ...
    sys.exit(stage("train-intime", ["bash", "examples/interact/screensim/run_screensim_train.sh",
...
if os.environ.get("INTERACT_INTERACTIVE_QUEUE") != "1":
    for name, argv, timeout in (
        ("rollout", [sys.executable, "examples/interact/screensim/screensim_rollout_probe.py"], 600),
        ("logprob_replay", [sys.executable, "examples/interact/common/check_rollout_logprobs.py"], 240),
        ("train", ["bash", "examples/interact/screensim/run_screensim_train.sh"], 1500),
    ):
